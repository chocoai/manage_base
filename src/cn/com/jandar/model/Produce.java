...
public class Produce extends Model<Produce> {
	...
	public static synchronized String saveAll(User user, Produce produce,
		...
		if(!checksepara(produce))
			return "存在仓库未填写信息，请联系管理员核实数据！";
...
//						}
	...
	public static boolean checksepara(Produce produce) {
		String dhlx = DbUtil.readDbString(produce.getStr("dhlx"));
		if ("001,002,008,010,012".indexOf(dhlx) != -1) {
			if("".equals(DbUtil.readDbString(produce.getStr("RKCKBH"))))
				return false;
		}else if("006".indexOf(dhlx) != -1){
			if("".equals(DbUtil.readDbString(produce.getStr("RKCKBH"))))
				return false;
			if("".equals(DbUtil.readDbString(produce.getStr("CKCKBH"))))
				return false;
		}else{
			if("".equals(DbUtil.readDbString(produce.getStr("CKCKBH"))))
				return false;
