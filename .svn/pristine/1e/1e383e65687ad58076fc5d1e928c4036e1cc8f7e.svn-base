...
@ControllerBind(controllerKey = "/manage/cuku/shckd", resource = "损耗出库单")
public class ShckdController extends AdminBaseController {
	...
	public void update() {
		...
		if (msg.equals(Produce.SUCCESS)) {
			...
			setAttr("redirectionUrl", "/manage/cuku/shckd");
	...
	public void print(){     
		...
		for(Record r : result) {
			...
			row.put("FNAME",DicPlugin.b_factorysimgle.get(r.get("FACTORYID") + ""));				//厂商名称
